...
system('cd ~')
system('mkdir enrichment_KEGG_old_all')
...
{
    t3 <- proc.time()
    gene.stats <- gene.loading.stats.1_layer_only(snp.svd$v[,layer],gene.data=gene.2.db,KEGG.data=KEGG.db)
    ...
    setwd("~/enrichment_KEGG_old_all/")
    save(gene.stats,result.KEGG,file=paste('result-KEGG-',layer,'.Rdata',sep=''))
    enrichment.output(result.KEGG, KEGG.db, num_terms=500, filename=paste('wilcox-molecular_function',layer,sep=''), p_cutoff=0.05)
